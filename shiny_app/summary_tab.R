...
output$data_explorer <- renderUI({
  ...
  subheading <- case_when(input$measure_select == "Admissions" ~ "COVID-19 related admissions have been identified as the following: A patient may have tested positive 
                          ...
                          input$measure_select == "SAS" ~ paste0("Please note: For the 27/01/21 release of this dashboard there was a data extraction issue involving the Unscheduled Care Datamart. As a result of this, the SAS outputs by Age Group and by SIMD will be complete to the week ending 17/01/21."))
